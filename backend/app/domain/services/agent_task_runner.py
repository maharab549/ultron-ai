...
class AgentTaskRunner(TaskRunner):
    ...
    async def _handle_tool_event(self, event: ToolEvent):
        """Generate tool content with enhanced error handling and logging"""
        try:
            if event.status != ToolStatus.CALLED:
                return

            logger.info(f"Processing tool event for {event.tool_name}")
            ...
            if event.tool_name == "browser":
                ...
                if event.function_result and hasattr(event.function_result, 'data'):
                    results = getattr(event.function_result.data, 'results', [])
                    event.tool_content = SearchToolContent(results=results)
                else:
                    event.tool_content = SearchToolContent(results=[])
            
            elif event.tool_name == "shell":
                shell_id = event.function_args.get("id")
                if shell_id:
                    shell_result = await self._sandbox.view_shell(shell_id, console=True)
                    console_output = shell_result.data.get("console", [])
                    event.tool_content = ShellToolContent(console=console_output)
                else:
                    event.tool_content = ShellToolContent(console=["(No Console ID provided)"])
                ...
                if file_path:
                    ...
                    event.tool_content = FileToolContent(content="(No file path provided)")
            
            elif event.tool_name == "mcp":
                if event.function_result:
                    result_data = "No result available"
                    if hasattr(event.function_result, 'data') and event.function_result.data:
                        result_data = event.function_result.data
                    elif hasattr(event.function_result, 'model_dump'):
            ...
            logger.exception(f"Agent {self._agent_id} failed to generate tool content for {event.tool_name}: {e}")
