...
class BaseAgent(ABC):
    ...
    async def execute(self, request: str, format: Optional[str] = None) -> AsyncGenerator[BaseEvent, None]:
        """Execute the agent with enhanced tool handling and error recovery"""
        ...
        for iteration in range(self.max_iterations):
            ...
            for tool_call in message["tool_calls"]:
                ...
                try:
                    ...
                    logger.exception(f"Error in tool execution loop: {e}")
                    yield ErrorEvent(error=f"Tool execution failed: {str(e)}")
