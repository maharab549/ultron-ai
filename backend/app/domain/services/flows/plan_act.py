...
class PlanActFlow(BaseFlow):
    ...
    async def run(self, message: Message) -> AsyncGenerator[BaseEvent, None]:
        ...
        if session.status == SessionStatus.COMPLETED:
            logger.info(f"Session {self._session_id} is already COMPLETED, restarting flow")
            self.status = AgentStatus.PLANNING
        elif session.status == SessionStatus.WAITING:
            logger.info(f"Session {self._session_id} is in WAITING status, resuming execution")
            self.status = AgentStatus.EXECUTING
        else:
            logger.info(f"Session {self._session_id} status is {session.status}, starting planning")
            self.status = AgentStatus.PLANNING
